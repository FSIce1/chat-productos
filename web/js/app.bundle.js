...
var initApp = (function(app) {
	...
	app.listFilter = function (list, input, anchor) {
		...
		$(input).change( function () {
			...
			if(filter.length > 1) {
				...
				if (listPrev){	
					listPrev.text("filtrado " + $(list).find('li.js-filter-show').length + " de " + $(list).find('[data-filter-tags]').length + " total");
				}
	...
	app.factoryReset = function () {
		...
		if (typeof bootbox  != 'undefined') {
			...
			bootbox.confirm({
				title: "<i class='fal fa-exclamation-triangle text-warning mr-2'></i> Está a punto de restablecer todas las configuraciones locales de almacenamiento",
				message: "<span><strong>Warning:</strong> Esta acción no es reversible. Perderá todas las configuraciones de diseño.</span>",
				centerVertical: true,
				swapButtonOrder: true,
				buttons: {
					confirm: {
						label: 'Restablecimiento de fábrica',
			...
			if (confirm( 'Está a punto de restablecer todo su almacenamiento local al estado nulo. ¿Desea continuar?' )) {
				localStorage.clear();
				initApp.resetSettings();
				location.reload();
			}
	...
	app.domReadyMisc = function() {
		...
		if ( myapp_config.appDateHook.length ) {
			var months = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Augosto', 'Septimbre', 'Octubre', 'Noviembre', 'Diciembre'],
				...
				switch ( true ) {
					...
					case ( actiontype === 'panel-close' ):
						...
						if (typeof bootbox  != 'undefined') {
							...
							bootbox.confirm({
								title: "<i class='fal fa-times-circle text-danger mr-2'></i> ¿Desea eliminar el panel?",
							...
							if (confirm( '¿Desea eliminar el panel? ' + selectedPanel.children('.panel-hdr').children('h2').text().trim() + '?' )) {
								killPanel();
							}
